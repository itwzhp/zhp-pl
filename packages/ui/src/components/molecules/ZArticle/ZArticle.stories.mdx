  ...
  <Story name="Common">
    {{
        ...
        props: {
          ...
          thumbnail: {
            default: text('thumbnail', 'http://demo.przemyslawspaczek.atthost24.pl/wp-content/uploads/2020/08/fot.-ZHP-_-Karolina-Piotrowska-18.jpg')
          },
          title: {
  ...
  <Story name="Grid">
    {{
      ...
      async mounted(){
        ...
        let users = await axios.get('http://demo.przemyslawspaczek.atthost24.pl/wp-json/wp/v2/users')
        users = users.data.reduce((accumulator, user) => ({...accumulator, [user.id]: {name: user.name, to: user.link}}),{});
        const {data} =  await axios.get('http://demo.przemyslawspaczek.atthost24.pl/wp-json/wp/v2/posts?per_page=16')
        // todo: create scoped-slot component
        const postWithMedia = await Promise.all(data.map(async(post) => {
          const {data} = await axios.get('http://demo.przemyslawspaczek.atthost24.pl/wp-json/wp/v2/media/'+post.featured_media)
